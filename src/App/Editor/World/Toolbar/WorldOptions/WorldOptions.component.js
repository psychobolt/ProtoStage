...
export default class WorldOptions extends React.Component<Props> {
  setGravityX = (event: CustomEvent, value: number) => {
    const { updateGravity } = this.props;
    updateGravity({ x: value });
  }

  setGravityY = (event: CustomEvent, value: number) => {
    const { updateGravity } = this.props;
    updateGravity({ y: value });
  }

  updateMouseForce = (event: CustomEvent, value: number) => {
    const { updateMouseForce } = this.props;
    updateMouseForce(value);
  }

  updateSpeed = (event: CustomEvent, value: number) => {
